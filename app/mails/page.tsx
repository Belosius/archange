...
// Renvoie le nom de la plateforme détectée ou null si c'est un client direct.
...
// Tente d'extraire email/téléphone/nom du corps quand l'expéditeur est une plateforme.
// Renvoie un objet { email, telephone, nom } avec les valeurs trouvées ou null.
function extraireContactDepuisCorps(corps: string, fromEmail?: string): { email: string|null, telephone: string|null, nomComplet: string|null } {
  ...
  // Email — toutes adresses sauf celle de la plateforme et adresses jetables
...
// Tarif Anthropic Claude Sonnet 4 : $3/M input, $15/M output (USD)
...
Beaucoup de demandes arrivent via des plateformes intermédiaires :
  ...
  - Notifications Google/Outlook ("via Google Forms", "via Microsoft Forms")
...
INDICES qu'un mail vient d'une plateforme :
  ...
  - Le corps contient des sections structurées type formulaire :
    ...
    "Demande de devis | Date: ... | Personnes: ..."
...
- isReservation : true UNIQUEMENT si l'email contient une demande explicite de réservation, privatisation, devis pour un groupe, ou un événement. Une simple question sur les horaires ou le menu = false.
  Note : un mail venant d'une plateforme de réservation (Zenchef, ABC Salles, etc.) avec des coordonnées client structurées est presque toujours isReservation=true, même si le mot "réservation" n'apparaît pas explicitement.
...
- nom : Prénom + NOM du client réel (jamais le nom de la plateforme).
  Format : "Prénom NOM" si les deux sont identifiables, sinon ce qui est disponible.
  Cherche dans le corps : "Nom:", "De la part de:", "Contact:", signatures de mail.

- email : adresse email du client réel.
  Si le "De:" est un noreply/plateforme → cherche dans le corps les patterns :
    "Email:", "Mail:", "Adresse électronique:", "Reply-To:", ou toute adresse email
    présente dans la signature ou les coordonnées du formulaire.
  Si tu trouves plusieurs emails, prends celui du client (pas celui de la plateforme,
  pas l'email de l'établissement destinataire).

- telephone : numéro de téléphone du client.
  Cherche : "Téléphone:", "Tél:", "Phone:", "Mobile:", ou tout numéro au format français
  (06/07 mobile, 01-05 fixe, +33). Ignore les numéros de la plateforme ou de l'établissement.

- entreprise : nom de la société du client (PAS le nom de la plateforme).
  Cherche : "Société:", "Entreprise:", "Company:", "Organisation:", domain de l'email
  (ex: roxane@setec.com → SETEC), signature email.
  Si le client est un particulier (pas de société mentionnée) → null.
...
- notes : résume en 1-2 phrases les détails importants non couverts par les autres champs. Si l'email est dans une autre langue que le français, indique-le ici. Si le mail vient d'une plateforme intermédiaire, mentionne-la ici (ex: "Demande reçue via Zenchef").

- statutSuggere : suggère un statut parmi [nouveau, en_cours, en_attente, confirme] selon le contenu du mail

- sourceEmail : indique d'où provient le mail. Valeurs possibles :
  "client_direct" si le client a écrit directement,
  "plateforme:zenchef" / "plateforme:abc_salles" / "plateforme:funbooker" / "plateforme:autre" si via plateforme,
  "formulaire_contact" si formulaire de site,
  null si indéterminé.
...
3. ✅ L'entreprise retournée est bien celle du client (pas la plateforme intermédiaire) ?
4. ✅ Si tu n'as pas trouvé l'email/téléphone/nom dans le corps : as-tu vraiment regardé toute la signature et les sections type "Coordonnées:" ?

Si un doute → relis le corps une seconde fois pour trouver les vraies coordonnées avant de finaliser.

JSON à retourner (exactement cette structure) :
{
  "isReservation": false,
...
// Construit le message email à passer en input (avec balises XML pour ancrage attention)
function buildExtractMessage(email: {
  ...
  // Briefing en début (attention max)
  const briefing = plateforme
    ? `🎯 BRIEFING : Mail reçu via la plateforme "${plateforme}". Le client réel n'est PAS l'expéditeur "${fromEmail}". Cherche ses vraies coordonnées dans le corps ci-dessous.${contactExtrait?.email ? `\n   Indice automatique : email candidat trouvé = ${contactExtrait.email}` : ""}${contactExtrait?.nomComplet ? `\n   Indice automatique : nom candidat trouvé = ${contactExtrait.nomComplet}` : ""}${contactExtrait?.telephone ? `\n   Indice automatique : téléphone candidat = ${contactExtrait.telephone}` : ""}\n   Vérifie ces indices et complète les autres champs en lisant le corps complet.`
    : `🎯 BRIEFING : Mail direct du client (pas de plateforme intermédiaire détectée).`;

  // Corps tronqué intelligemment (Option A : 30 000 chars)
...
// Construit le message complet pour la génération de réponse — structure XML balisée
function buildResponseMessage(opts: {
  ...
  // Vrai contact (priorité : extracted.email > détection corps > fromEmail)
...
${plateforme ? `📨 Mail reçu via la plateforme "${plateforme}" (ne pas saluer la plateforme — saluer ${prenom})` : "📨 Mail direct du client"}
...
${extracted?.typeEvenement ? `Type d'événement : ${extracted.typeEvenement}` : ""}
${extracted?.budget ? `Budget mentionné : ${extracted.budget}` : ""}
${extracted?.resume ? `Résumé : ${extracted.resume}` : ""}`;

  // Historique des échanges (filtré et trié, max 5 mails)
...
${historiqueMails.slice(0, 5).map(h => {
  ...
  // Corps du mail courant — la pièce critique, en avant-dernière position (récence bias)
...
2. ✅ N'utilise QUE les tarifs/conditions présents dans <sources_archange> du contexte système ci-dessus — n'invente jamais un chiffre
3. ✅ Vérifie la disponibilité dans <planning_temps_reel> avant de confirmer une date/un espace
4. ✅ Si l'historique montre une promesse déjà faite, respecte-la (ne contredis pas)
...
async function callClaude(msg: string, system: string, docs: any[] | null, callType: string = "generic"): Promise<string> {
  ...
  try {
    ...
    // Tracking — préférer les vrais tokens si l'API les renvoie, sinon estimation
...
export default function App() {
  ...
  // Refresh des stats toutes les 2 sec (lit depuis apiUsageStats global)
  useEffect(() => {
  ...
  const getLinkedEmails = React.useCallback((resa: any) => {
    ...
    return emails.filter(m => {
      ...
      // Permet de lier un mail Zenchef/ABC Salles au bon dossier client
      if (resaEmailLower) {
    ...
    try {
      ...
      const mailsRecusDuClient = emails
        .filter(m => m.id !== sel.id && m.fromEmail && (
          ...
          // Aussi : un mail dont l'extracted indique le même vrai client
      ...
      const infoRaw = settled[1]; // undefined si on n'a pas appelé l'extraction
    ...
    <div style={{display:"flex",height:"100vh",overflow:"hidden",fontFamily:"'Inter','DM Sans','Helvetica Neue',sans-serif",background:"#F7F2EA"}}>
        ...
        <div style={{position:"fixed",bottom:12,right:16,zIndex:9998}}>
          ...
          {apiStatsOpen && (
            <div style={{
              ...
              boxShadow:"0 8px 24px rgba(15,15,20,0.1)",
              fontSize:12,
              color:"#1A1A1E",
              fontFamily:"'Geist','system-ui',sans-serif",
            }}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"6px 14px",fontVariantNumeric:"tabular-nums"}}>
                <span style={{color:"#6B6B72"}}>Appels API :</span>
                <span style={{textAlign:"right",fontWeight:500}}>{apiStatsView.totalCalls}</span>
                <span style={{color:"#6B6B72"}}>Tokens entrée :</span>
                <span style={{textAlign:"right",fontWeight:500}}>{apiStatsView.totalInputTokens.toLocaleString("fr-FR")}</span>
                <span style={{color:"#6B6B72"}}>Tokens sortie :</span>
                <span style={{textAlign:"right",fontWeight:500}}>{apiStatsView.totalOutputTokens.toLocaleString("fr-FR")}</span>
                <span style={{color:"#6B6B72",borderTop:"1px solid #EBEAE5",paddingTop:6}}>Coût session :</span>
                <span style={{textAlign:"right",fontWeight:600,color:"#B8924F",borderTop:"1px solid #EBEAE5",paddingTop:6}}>${apiStatsView.totalCostUSD.toFixed(4)}</span>
                <span style={{color:"#6B6B72"}}>Coût en € (~) :</span>
                <span style={{textAlign:"right",fontWeight:500}}>≈ {(apiStatsView.totalCostUSD * 0.92).toFixed(4)} €</span>
              </div>
              <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #EBEAE5",fontSize:11,color:"#A5A4A0",lineHeight:1.4}}>
                Compteur depuis le chargement de la page. Tarif Claude Sonnet 4 : $3/M tokens entrée, $15/M sortie.
              </div>
              <button onClick={()=>setApiStatsOpen(false)} style={{marginTop:8,width:"100%",padding:"6px 0",background:"none",border:"1px solid #EBEAE5",borderRadius:6,color:"#6B6B72",fontFamily:"inherit",fontSize:11,cursor:"pointer"}}>
                Fermer
              </button>
            </div>
          )}
      ...
      <main style={{flex:1,display:"flex",overflow:"hidden",minWidth:0}}>
        ...
        {view==="mails" && (
          <>
            ...
            {(mailFilter!=="priorites" || (mailFilter==="priorites" && sel)) && <div style={{flex:1,overflowY:"auto",background:"#F7F2EA"}}>
              {!sel ? (
                    ...
                    {dayResas.length===0?(
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:12}}>
                        {dayResas.map(r=>{ const st=getStatut(r); return (
                          <div key={r.id} onClick={()=>{ setSelResaGeneral(r); setResaOnglet("infos"); }} style={{background:"#FFFFFF",borderRadius:3,border:"1px solid #E6DCC9",borderLeft:`4px solid ${st.color}`,padding:"16px 18px",cursor:"pointer"}}>
      ...
      {selResaGeneral && !editResaPanel && (
        <div style={{position:"fixed",inset:0,zIndex:600,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}} onClick={e=>{if(e.target===e.currentTarget){setSelResaGeneral(null);setResaOnglet("infos");}}}>
          ...
          <div style={{position:"relative",width:580,maxWidth:"95vw",height:"100vh",background:"#FDFCFA",display:"flex",flexDirection:"column",boxShadow:"-8px 0 40px rgba(0,0,0,0.15)",borderLeft:"1px solid #E6DCC9"}}>
            ...
            <div style={{padding:"20px 24px 16px",borderBottom:"1px solid #E6DCC9",flexShrink:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:14}}>
                ...
                <button onClick={()=>{setSelResaGeneral(null);setResaOnglet("infos");}} style={{width:26,height:26,borderRadius:6,border:"1px solid #E6DCC9",background:"transparent",color:"#6B6E7E",cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>×</button>
              </div>
            </div>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  ...
                  <div>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                      {statuts.map(s=>(
                        <button key={s.id} onClick={()=>{const upd=resas.map(r=>r.id===selResaGeneral.id?{...r,statut:s.id}:r);saveResas(upd);setSelResaGeneral({...selResaGeneral,statut:s.id});}} style={{padding:"6px 15px",borderRadius:100,border:"none",background:(selResaGeneral.statut||"nouveau")===s.id?s.bg:"#F0EDE8",color:(selResaGeneral.statut||"nouveau")===s.id?s.color:"#6B6E7E",fontSize:12,fontWeight:(selResaGeneral.statut||"nouveau")===s.id?700:500,cursor:"pointer"}}>{s.label}</button>
                      ))}
                    </div>
                  </div>
                  {/* Grille infos */}
                  ...
                  {noteIA[selResaGeneral.id]
                    ? <div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                          ...
                          <button onClick={()=>generateNoteIA(selResaGeneral)} disabled={!!genNoteIA} style={{fontSize:11,padding:"5px 12px",borderRadius:6,border:"1px solid #E6DCC9",background:"transparent",color:"#6B6E7E",cursor:genNoteIA?"wait":"pointer"}}>↺ Régénérer</button>
                        </div>
                        <div style={{fontSize:13,color:"#1B1E2B",lineHeight:1.75,whiteSpace:"pre-wrap"}}>{noteIA[selResaGeneral.id].text}</div>
                      </div>
                    : <div style={{textAlign:"center",padding:"40px 0"}}>
                        ...
                        <button onClick={()=>generateNoteIA(selResaGeneral)} disabled={!!genNoteIA} style={{padding:"10px 20px",borderRadius:8,border:"none",background:"#1B1E2B",color:"#B89456",fontSize:12,fontWeight:600,cursor:genNoteIA?"wait":"pointer",display:"flex",alignItems:"center",gap:8,margin:"0 auto"}}>
              ...
              {resaOnglet==="relances"&&(
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <div style={{fontSize:13,fontWeight:600,color:"#1B1E2B"}}>Relances planifiées</div>
                    <button onClick={()=>setShowRelanceForm(selResaGeneral.id)} style={{fontSize:11,padding:"6px 12px",borderRadius:6,border:"1px solid #E6DCC9",background:"transparent",color:"#6B6E7E",cursor:"pointer"}}>+ Ajouter</button>
                  </div>
                  {relances.filter(r=>r.resaId===selResaGeneral.id).length===0
                    ...
                    : [...relances.filter(r=>r.resaId===selResaGeneral.id)].sort((a,b)=>(a.date||"").localeCompare(b.date||"")).map(rel=>(
                        <div key={rel.id} style={{padding:"10px 14px",background:"#F7F2EA",borderRadius:8,border:"1px solid #E6DCC9",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div>
                            <div style={{fontSize:12,fontWeight:600,color:"#1B1E2B"}}>{rel.date}{rel.heure&&` à ${rel.heure}`}</div>
                  ...
                  {showRelanceForm===selResaGeneral.id&&(
                    <div style={{background:"#F7F2EA",borderRadius:10,padding:"16px",border:"1px solid #E6DCC9",marginTop:12}}>
                  ...
                  <button onClick={()=>{setRelanceIAText("");setMotifSelectionne("");setMotifPersonnalise("");setShowRelanceIA(selResaGeneral);}} style={{width:"100%",padding:"10px",borderRadius:8,border:"none",background:"#1B1E2B",color:"#B89456",fontSize:12,fontWeight:600,cursor:"pointer",marginTop:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>✨ Générer une relance IA</button>
                </div>
              )}
            </div>
      ...
      {editResaPanel&&selResaGeneral&&(
        <div style={{position:"fixed",inset:0,zIndex:601,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
          ...
          <div style={{position:"relative",width:420,maxWidth:"95vw",height:"100vh",background:"#FDFCFA",display:"flex",flexDirection:"column",overflowY:"auto",boxShadow:"-8px 0 40px rgba(0,0,0,0.2)",borderLeft:"1px solid #E6DCC9"}}>
            <div style={{padding:"16px 18px 12px",borderBottom:"1px solid #E6DCC9",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
