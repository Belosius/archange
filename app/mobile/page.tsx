...
// ═══════════════════════════════════════════════════════════════════════════
// CONSTANTES — partagées avec la version desktop (mêmes tables Supabase)
// ═══════════════════════════════════════════════════════════════════════════
interface EspaceDyn {
...
const DEFAULT_ESPACES_DYN: EspaceDyn[] = [
  { id: "rdc",       nom: "Rez-de-chaussée", color: "#C9A876", assisMin: "80",  assisMax: "100", deboutMin: "100", deboutMax: "150", description: "" },
  { id: "patio",     nom: "Le Patio",         color: "#6DB8A0", assisMin: "40",  assisMax: "75",  deboutMin: "60",  deboutMax: "100", description: "" },
  { id: "belvedere", nom: "Le Belvédère",     color: "#6D9BE8", assisMin: "40",  assisMax: "75",  deboutMin: "60",  deboutMax: "100", description: "" },
];
...
const DEFAULT_STATUTS: StatutDef[] = [
  ...
  { id: "confirme",   label: "Confirmé",   bg: "#EDF2E8", color: "#3F5B32" },
  { id: "annule",     label: "Annulé",     bg: "#FAEDEB", color: "#A84B45" },
];
...
// Split nom/prenom pour rétrocompat (résas anciennes sans prenom)
...
function humanError(e: any): string {
  ...
  if (m.includes("network") || m.includes("failed to fetch")) return "Connexion internet perdue. Réessayez.";
  if (m.includes("rate") || m.includes("429")) return "Trop de requêtes. Attendez quelques secondes.";
  if (m.includes("401") || m.includes("unauthorized")) return "Session expirée. Reconnectez-vous.";
  if (m.includes("timeout")) return "Délai dépassé. Réessayez.";
  return "Une erreur est survenue. Réessayez.";
}

// ═══════════════════════════════════════════════════════════════════════════
// TOKENS DESIGN — palette Apple Mail 2026 aéré
// ═══════════════════════════════════════════════════════════════════════════
const T = {
  bgCanvas: "#FAFAF7",
  bgSurface: "#FFFFFF",
...
export default function MobileApp() {
  ...
  // Navigation globale entre onglets + sous-écrans
  ...
  // ──── Data partagée avec desktop (Supabase) ────
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // CHARGEMENT INITIAL — même endpoint que desktop
  // ═══════════════════════════════════════════════════════════════════════════
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        // Restauration (mêmes champs que desktop)
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SAVE HELPERS — écrivent dans Supabase (partagé avec desktop)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const saveEmails = (e: any[]) => { setEmails(e); /* emails ne sont pas persistés — toujours rechargés depuis Gmail */ };
  ...
  const filteredMails = useMemo(() => {
    ...
    let list = emails.filter(m => {
      // Snoozés masqués sauf si filtre snoozed
  ...
  const generateWithArchange = async () => {
    if (!composeInReplyTo) { showToast("Impossible de générer sans mail source", "err"); return; }
    setComposeGenerating(true);
    try {
      const system = `Tu es ARCHANGE, l'agent email de ${nomEtab || "notre établissement"}.
...
Rédige une réponse chaleureuse et professionnelle au mail suivant. Signe "ARCHANGE".`;
      const msg = `Mail reçu :
...
${composeInReplyTo.body || composeInReplyTo.snippet}`;
  ...
  const createEvent = () => {
    ...
    if (!newEvent.heureDebut) errs.heureDebut = "Heure de début";
  ...
  const deleteEvent = (resa: any) => {
    if (!window.confirm(`Supprimer l'événement de ${displayNom(resa)} ?`)) return;
    ...
    return (
      <div style={{minHeight:"100vh",background:T.bgCanvas,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:T.fontSans}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontFamily:T.fontSerif,fontSize:40,color:T.accent,marginBottom:16}}>✦</div>
          <div style={{fontSize:13,color:T.textSecondary}}>Chargement…</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{minHeight:"100vh",background:T.bgCanvas,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:T.fontSans,padding:24}}>
        <div style={{textAlign:"center",maxWidth:280}}>
          <div style={{fontFamily:T.fontSerif,fontSize:48,color:T.accent,marginBottom:12}}>✦</div>
          <div style={{fontFamily:T.fontSerif,fontSize:26,color:T.textPrimary,marginBottom:8,letterSpacing:"-0.02em"}}>ARCHANGE</div>
          <div style={{fontSize:14,color:T.textSecondary,marginBottom:24,lineHeight:1.5}}>Connectez-vous pour accéder à vos mails et votre planning.</div>
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SOUS-ÉCRANS (plein écran, couvrent toute l'app)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // RENDU PRINCIPAL — bottom nav + onglet actif
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  function renderPlanning() {
    const headerSub =
      planView === "jour" ? `${fmtDateLongue(fmtISO(planDate))} · ${dayResas.length} événement${dayResas.length>1?"s":""}` :
      planView === "semaine" ? `Semaine du ${weekDays[0].getDate()} au ${weekDays[6].getDate()} ${MOIS[weekDays[6].getMonth()].toLowerCase()}` :
      `${MOIS[planDate.getMonth()]} ${planDate.getFullYear()} · ${planKPIs.total} événement${planKPIs.total>1?"s":""}`;

    const periodLabel =
      planView === "jour" ? fmtDateLongue(fmtISO(planDate)) :
      planView === "semaine" ? `${weekDays[0].getDate()} — ${weekDays[6].getDate()} ${MOIS[weekDays[6].getMonth()].slice(0,3)}` :
      ...
      <>
        ...
        <div style={{padding:"0 20px 14px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          ...
          <div style={kpiCardStyle()}>
            ...
            <div style={kpiLabelStyle()}>{planView==="jour"?"Invités":"Confirmés"}</div>
          </div>
          <div style={kpiCardStyle()}>
    ...
    if (dayResas.length === 0) {
      return (
        <div style={{textAlign:"center",padding:"48px 20px",color:T.textTertiary}}>
          ...
          <div style={{fontSize:14,color:T.textSecondary}}>Aucun événement ce jour</div>
          <div style={{fontSize:12,marginTop:4}}>Appuyez sur ＋ pour ajouter une demande</div>
        </div>
      );
    }
    return (
            ...
            <div key={ds} style={{marginBottom:18}}>
              <div style={{display:"flex",alignItems:"baseline",gap:8,padding:"8px 4px 10px",position:"sticky",top:0,background:T.bgCanvas,zIndex:2}}>
                ...
                <div style={{fontSize:12.5,color:T.textSecondary,fontWeight:500,lineHeight:1.1}}>
                  <div style={{color:isTd?T.accent:T.textPrimary,fontSize:13.5,letterSpacing:"-0.005em",marginBottom:1,fontWeight:500}}>{JOURS_LONG[d.getDay()]}{isTd?" · Aujourd'hui":""}</div>
                  <span>{rs.length} événement{rs.length!==1?"s":""}{totalPers?` · ${totalPers} pers.`:""}</span>
                </div>
              ...
              {rs.length === 0 ? (
                <div style={{padding:"10px 4px 0",fontSize:13,color:T.textTertiary}}>—</div>
              ) : (
      ...
      <>
        ...
        <div style={{marginTop:14,background:T.bgSurface,borderTop:`1px solid ${T.borderHairline}`}}>
          <div style={{padding:"14px 20px 10px",display:"flex",alignItems:"baseline",justifyContent:"space-between",borderBottom:`1px solid ${T.borderHairline}`}}>
            ...
            <div style={{fontSize:11,color:T.textTertiary,textTransform:"uppercase",letterSpacing:"0.06em",fontWeight:500,fontVariantNumeric:"tabular-nums"}}>{selectedDayResas.length} événement{selectedDayResas.length!==1?"s":""}</div>
          </div>
    ...
    return (
      <>
        ...
        <div style={{padding:"4px 16px 10px",background:T.bgCanvas,position:"relative"}}>
          ...
          <input value={mailSearch} onChange={e=>setMailSearch(e.target.value)} placeholder="Rechercher un mail…" style={{width:"100%",padding:"9px 14px 9px 36px",border:`1px solid ${T.borderHairline}`,borderRadius:10,background:T.bgSurface,fontFamily:T.fontSans,fontSize:14,color:T.textPrimary,outline:"none"}}/>
        </div>
        ...
        <div style={{display:"flex",gap:7,padding:"0 16px 12px",overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          {([
            ...
            ["star", "Favoris", null],
            ["atraiter", "À traiter", countATraiter],
            ["snoozed", "Reportés", countSnoozed],
            ["archived", "Archivés", null],
    ...
    return (
      <div style={frameStyle}>
        ...
        <div style={{padding:"6px 12px 0",background:T.bgCanvas,display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          ...
          <div style={{fontSize:14,color:T.textSecondary,fontWeight:500,flex:1,textAlign:"center"}}>Boîte de réception</div>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:90}}>
          ...
          {/* Card résa détectée (si applicable) */}
          {isReservation && (
            <div style={{margin:"14px 16px 0",background:T.sageSubtle,border:`1px solid rgba(107,138,91,0.25)`,borderRadius:12,overflow:"hidden"}}>
              <div style={{padding:"10px 14px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid rgba(107,138,91,0.15)"}}>
                ...
                <div style={{fontSize:12,fontWeight:500,color:T.sageDark,textTransform:"uppercase",letterSpacing:"0.06em",flex:1}}>Réservation détectée par ARCHANGE</div>
              ...
              <div style={{padding:"12px 14px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {extracted.dateDebut && <ResaField label="Date" value={fmtDateFr(extracted.dateDebut) + (extracted.heureDebut ? ` · ${extracted.heureDebut}` : "")}/>}
                {extracted.nombrePersonnes && <ResaField label="Personnes" value={extracted.nombrePersonnes}/>}
                {extracted.espaceSuggere && <ResaField label="Espace" value={espaces.find(e=>e.id===extracted.espaceSuggere)?.nom || "—"}/>}
          ...
          <div style={{padding:"18px 20px 24px",fontSize:14.5,lineHeight:1.6,color:T.textPrimary,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>
            {mail.body || mail.snippet || "Chargement du contenu…"}
          </div>
        </div>
        ...
        <div style={{background:"rgba(255,255,255,0.96)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderTop:`1px solid ${T.borderHairline}`,padding:"10px 14px 26px",display:"flex",gap:8,flexShrink:0}}>
          <button onClick={()=>{ openCompose(mail); setTimeout(()=>generateWithArchange(), 100); }} style={{flex:1,padding:"12px 0",borderRadius:11,border:"1px solid #1A1A1E",background:"#1A1A1E",color:"#FFFFFF",fontFamily:T.fontSans,fontSize:13.5,fontWeight:500,display:"inline-flex",alignItems:"center",justifyContent:"center",gap:6,cursor:"pointer"}}>
            <span style={{color:T.accent}}>✦</span> Répondre avec ARCHANGE
          </button>
    ...
    return (
      <div style={frameStyle}>
        ...
        <div style={{display:"flex",alignItems:"center",padding:"6px 8px 10px",background:T.bgCanvas,borderBottom:`1px solid ${T.borderHairline}`,flexShrink:0}}>
          ...
          <div style={{flex:1,textAlign:"center",fontSize:15,fontWeight:500,color:T.textPrimary}}>{composeInReplyTo ? "Réponse" : "Nouveau mail"}</div>
        ...
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{background:T.bgSurface,borderBottom:`1px solid ${T.borderHairline}`,flexShrink:0}}>
            <div style={composeRowStyle()}>
              <span style={composeLabelStyle()}>À</span>
          ...
          {composeInReplyTo && (
            <div style={{padding:"14px 18px",background:T.bgCanvas,flexShrink:0}}>
              <button onClick={generateWithArchange} disabled={composeGenerating} style={{width:"100%",padding:"11px 14px",background:"linear-gradient(135deg, rgba(184,146,79,0.12) 0%, rgba(184,146,79,0.06) 100%)",border:"1px solid rgba(184,146,79,0.3)",borderRadius:11,color:T.accent,fontFamily:T.fontSans,fontSize:13.5,fontWeight:500,display:"inline-flex",alignItems:"center",justifyContent:"center",gap:8,letterSpacing:"-0.005em",opacity:composeGenerating?0.6:1,cursor:composeGenerating?"wait":"pointer"}}>
                {composeGenerating ? (
                  <><span style={{fontSize:16}}>✦</span> Génération en cours…</>
                ) : (
                  <><span style={{fontSize:16}}>✦</span> Générer une réponse avec ARCHANGE</>
                )}
              </button>
            </div>
          )}
          ...
          {/* Corps */}
          <textarea value={composeBody} onChange={e=>setComposeBody(e.target.value)} placeholder="Écrivez votre message…" style={{flex:1,padding:"16px 18px",background:T.bgSurface,fontFamily:T.fontSans,fontSize:14.5,lineHeight:1.55,color:T.textPrimary,border:"none",outline:"none",resize:"none",width:"100%",minHeight:200}}/>
        </div>
      </div>
    );
  }
  ...
  function renderEvents() {
    return (
      <>
        <div style={{padding:"6px 20px 14px",background:T.bgCanvas,borderBottom:`1px solid ${T.borderHairline}`}}>
          <div style={{fontFamily:T.fontSerif,fontSize:30,fontWeight:400,color:T.textPrimary,letterSpacing:"-0.025em",lineHeight:1.05}}>Événements</div>
          <div style={{fontSize:13,color:T.textSecondary,marginTop:4,fontVariantNumeric:"tabular-nums"}}>{resas.length} demande{resas.length!==1?"s":""}{eventKPIs.aRelancer>0?` · ${eventKPIs.aRelancer} à relancer`:""}</div>
        </div>
        ...
        <div style={{padding:"14px 20px 14px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:7}}>
          ...
          <div style={kpiCardStyle(true)}>
            ...
            <div style={{...kpiLabelStyle(),fontSize:10}}>À relancer</div>
          </div>
          <div style={kpiCardStyle(true)}>
          ...
          <div style={kpiCardStyle(true)}>
            ...
            <div style={{...kpiLabelStyle(),fontSize:10}}>Prévi.</div>
          </div>
        </div>
  ...
  function renderEventsByUrgency() {
    const groups: [string, string, any[]][] = [
      ["Dans −7 jours", T.danger, eventsByUrgency.urgent],
      ["Relances sans réponse +3 jours", T.warn, eventsByUrgency.relancer],
      ...
      ["Hors réservation", T.textQuat, eventsByUrgency.hors],
    ];
    const hasAny = groups.some(([,,list]) => list.length > 0);
    if (!hasAny) {
      return <div style={{textAlign:"center",padding:"48px 20px",color:T.textTertiary,fontSize:14}}>Aucun événement</div>;
    }
    return (
      <>
    ...
    return (
      <div style={frameStyle}>
        ...
        <div style={{padding:"6px 12px 0",background:T.bgCanvas,display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          ...
          <div style={{fontSize:14,color:T.textSecondary,fontWeight:500,flex:1,textAlign:"center"}}>{resa.dateDebut ? fmtDateLongue(resa.dateDebut) : "Événement"}</div>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:110}}>
          ...
          <div style={settingsGroupStyle()}>
            <div style={settingsGroupTitleStyle()}>Événement</div>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><rect x="1.5" y="2.5" width="11" height="10" rx="1.2" stroke="currentColor" strokeWidth="1.4"/><path d="M1.5 5.5h11" stroke="currentColor" strokeWidth="1.4"/></svg>} label="Date" value={resa.dateDebut ? fmtDateLongue(resa.dateDebut) : "—"}/>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="5.5" stroke="currentColor" strokeWidth="1.4"/><path d="M7 4v3l2 1.3" stroke="currentColor" strokeWidth="1.4"/></svg>} label="Horaires" value={resa.heureDebut ? `${resa.heureDebut}${resa.heureFin ? " → "+resa.heureFin : ""}` : "—"}/>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="4.5" r="2.5" stroke="currentColor" strokeWidth="1.4"/><path d="M2.5 12c0-2.5 2-4 4.5-4s4.5 1.5 4.5 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>} label="Personnes" value={resa.nombrePersonnes || "—"}/>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><path d="M7 12.5s3.8-3.4 3.8-7A3.8 3.8 0 107 1.7c0 3.5 3.8 7 3.8 7z" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>} label="Espace" value={espaceNom || "—"}/>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><path d="M10 3H5a2 2 0 000 4h4a2 2 0 010 4H3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M6.5 1.5v11" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>} label="Budget" value={resa.budget || "—"} valueColor={resa.budget ? T.accent : undefined}/>
            <InfoRow icon={<svg width="15" height="15" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="3.5" stroke="currentColor" strokeWidth="1.4"/></svg>} label="Type" value={resa.typeEvenement || "—"}/>
          </div>
            ...
            <div style={settingsGroupStyle()}>
              ...
              {resa.telephone && (
                <a href={`tel:${resa.telephone}`} style={{display:"flex",alignItems:"center",padding:"12px 16px",borderTop:`1px solid ${T.borderHairline}`,minHeight:52,textDecoration:"none",color:"inherit"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,color:T.textTertiary,marginBottom:2}}>Téléphone</div>
          ...
          <div style={{background:T.bgSurface,border:`1px solid ${T.borderHairline}`,borderRadius:12,margin:"12px 16px 0",overflow:"hidden"}}>
            <div style={{padding:"10px 14px",background:T.bgCanvas,borderBottom:`1px solid ${T.borderHairline}`,fontSize:11.5,fontWeight:500,color:T.textPrimary,display:"flex",alignItems:"center",gap:6}}>
              ...
              Note directeur
              <span style={{marginLeft:"auto",fontSize:10,color:T.textTertiary,textTransform:"uppercase",letterSpacing:"0.08em",fontWeight:500}}>Privé</span>
            </div>
            ...
            }} placeholder="Note confidentielle…" rows={3} style={{width:"100%",padding:"10px 14px",border:"none",background:T.bgSurface,fontFamily:T.fontSans,fontSize:13,lineHeight:1.6,color:T.textPrimary,outline:"none",resize:"vertical"}}/>
          </div>
        </div>
          ...
          {resa.email && (
            <button onClick={()=>{
              openCompose({
                ...
                subject: "Votre événement du " + (resa.dateDebut ? fmtDateFr(resa.dateDebut) : ""),
              });
      ...
      <div style={{padding:"14px 16px 0"}}>
        ...
        <FormGroup>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div>{inp("prenom","Prénom")}{errors.prenom&&<ErrMsg msg={errors.prenom}/>}</div>
            <div>{inp("nom","Nom")}{errors.nom&&<ErrMsg msg={errors.nom}/>}</div>
          </div>
          {inp("entreprise","Société (optionnel)")}
        </FormGroup>
        ...
        <FormGroup>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div>{inp("heureDebut","Début","time")}{errors.heureDebut&&<ErrMsg msg={errors.heureDebut}/>}</div>
        ...
        <FormGroup>
          ...
          <div style={{height:10}}/>
          {inp("budget","Budget (ex: 5 000€)")}
        </FormGroup>

        {/* NOTES */}
        <FormGroup>
          <FormLabel text="Notes"/>
          <textarea value={val.notes||""} onChange={e=>setVal({...val,notes:e.target.value})} rows={3} placeholder="Informations complémentaires…" style={{width:"100%",padding:"12px 14px",border:`1px solid ${T.borderHairline}`,borderRadius:10,background:T.bgSurface,fontFamily:T.fontSans,fontSize:15,color:T.textPrimary,outline:"none",resize:"vertical",minHeight:80,lineHeight:1.5}}/>
        </FormGroup>
      </div>
    );
  }
      ...
      <>
        ...
        <div style={settingsGroupStyle()}>
          <SettingsRow
            ...
            iconColor="gold"
            title={nomEtab || "Établissement"}
            subtitle="Profil établissement"
        ...
        <div style={settingsGroupStyle()}>
          ...
          <SettingsRow
            ...
            subtitle="Annulation, acomptes, délais"
          ...
          <SettingsRow
            ...
            iconColor="sage"
            title="Règles & ton ARCHANGE"
            subtitle="Signature, style de réponse"
          ...
          <SettingsRow
            ...
            iconColor="purple"
            title="Espaces"
            subtitle={espaces.map(e=>e.nom).join(" · ")}
          ...
          <SettingsRow
            ...
            iconColor="purple"
            title="Statuts"
            subtitle={`${statuts.length} statuts configurés`}
        ...
        <div style={settingsGroupStyle()}>
          ...
          <SettingsRow
            ...
            title="À propos d'ARCHANGE"
            subtitle="v1.0 · Anthropic"
            onClick={()=>{}}
          />
        </div>
  ...
  function renderAccountSub(key: string) {
    const configs: Record<string, {title:string, value:string, save:(v:string)=>void, placeholder:string}> = {
      menus: {title:"Menus & tarifs", value:menusCtx, save:(v)=>{setMenusCtx(v);saveSources({menus:v});}, placeholder:"Vos menus, formules, tarifs par personne, options boissons…"},
      conditions: {title:"Conditions & politique", value:conditionsCtx, save:(v)=>{setConditionsCtx(v);saveSources({conditions:v});}, placeholder:"Politique d'annulation, acomptes, délais de confirmation…"},
      ton: {title:"Règles & ton ARCHANGE", value:tonCtx, save:(v)=>{setTonCtx(v);saveSources({ton:v});}, placeholder:"Ex: Toujours proposer une visite. Signature personnalisée…"},
    };
    const cfg = configs[key];
    if (!cfg) {
      // Espaces / Statuts / Établissement — pour simplifier en v1 on renvoie vers desktop
      return (
        <div style={frameStyle}>
          <Toast toast={toast}/>
          <div style={{padding:"6px 12px 0",background:T.bgCanvas,display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center",flexDirection:"column"}}>
            ...
            <div style={{fontSize:14,color:T.textSecondary,marginBottom:24,lineHeight:1.5,maxWidth:260}}>L'édition de {key === "espaces" ? "vos espaces" : key === "statuts" ? "vos statuts" : "votre profil établissement"} est disponible sur la version desktop.</div>
...
function BottomNav({tab, setTab, countUnread, countATraiter}: {tab:string,setTab:any,countUnread:number,countATraiter:number}) {
  const items = [
    ...
    {id:"events", label:"Événements", icon:<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><rect x="3" y="3" width="14" height="14" rx="2" stroke="currentColor" strokeWidth="1.5"/><path d="M6 8h8M6 11h8M6 14h5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>, badge:null},
    ...
    <div onClick={onClick} style={{background:T.bgSurface,border:`1px solid ${T.borderHairline}`,borderRadius:14,padding:"14px 14px",marginBottom:10,boxShadow:"0 1px 2px rgba(15,15,20,0.03)",cursor:"pointer"}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:11,marginBottom:r.nombrePersonnes||espaceNom||r.budget?10:0}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-start",minWidth:52,flexShrink:0,paddingTop:2}}>
          ...
          {r.heureFin && <div style={{fontSize:12,color:T.textTertiary,marginTop:4,fontVariantNumeric:"tabular-nums"}}>→ {r.heureFin}</div>}
        </div>
  ...
  return (
    <div onClick={onClick} style={{background:T.bgSurface,border:`1px solid ${T.borderHairline}`,borderRadius:12,padding:"11px 12px",marginBottom:8,boxShadow:"0 1px 2px rgba(15,15,20,0.03)",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-start",minWidth:46,flexShrink:0}}>
        <div style={{fontFamily:T.fontSerif,fontSize:17,fontWeight:500,color:T.textPrimary,letterSpacing:"-0.02em",lineHeight:1,fontVariantNumeric:"tabular-nums"}}>{r.heureDebut||"—"}</div>
        {r.heureFin && <div style={{fontSize:11,color:T.textTertiary,marginTop:3,fontVariantNumeric:"tabular-nums"}}>→ {r.heureFin}</div>}
      </div>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:12,color:T.textSecondary,marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
          {[r.entreprise, r.nombrePersonnes ? `${r.nombrePersonnes} pers.` : null, espaceNom].filter(Boolean).join(" · ") || "—"}
        </div>
      </div>
...
function EventCardList({r, statuts, espaces, onClick}: any) {
  const st = statuts.find((s:any)=>s.id===(r.statut||"nouveau")) || statuts[0];
  const espaceNom = espaces.find((e:any)=>e.id===r.espaceId)?.nom || "";
  const dateLabel = r.dateDebut ? fmtDateFr(r.dateDebut).replace(/\./,"").substring(0, 10) : "—";
...
function ErrMsg({msg}: {msg:string}) {
  return <div style={{fontSize:11,color:T.danger,marginTop:4,paddingLeft:4}}>⚠ {msg}</div>;
}
