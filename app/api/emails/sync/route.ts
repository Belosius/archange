...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const listRes = await fetch(url, { headers: { Authorization: `Bearer ${session.accessToken}` } })
    ...
    if (!messages.length) return NextResponse.json({ synced: 0, nextPageToken: null, total: 0 })
    const BATCH = 20
    const rows: any[] = []
    for (let i = 0; i < messages.length; i += BATCH) {
      ...
      for (const msg of details) {
        ...
        rows.push({
          ...
          from_addr: header(h, 'from'), to_addr: header(h, 'to'),
          subject: header(h, 'subject') || '(sans objet)',
          date_iso: fmtDate(msg.internalDate), internal_date: Number(msg.internalDate),
          snippet: msg.snippet || '', body_text: text.slice(0, 50000), body_html: html.slice(0, 200000),
          labels: msg.labelIds || [], has_attachments: !!(msg.payload?.parts?.some((p: any) => p.filename)),
        })
      }
    }
    if (rows.length > 0) {
      const { error } = await supabase.from('emails_cache').upsert(rows, { onConflict: 'gmail_id,user_id', ignoreDuplicates: true })
      if (error) console.error('Supabase upsert error:', error)
    }
    return NextResponse.json({ synced: rows.length, nextPageToken: nextPageToken || null, total: resultSizeEstimate || 0 })
  } catch (e: any) {
...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabase.from('emails_cache').select('date_iso').eq('user_id', session.user.id).order('internal_date', { ascending: false }).limit(1).single()
  const { count } = await supabase.from('emails_cache').select('*', { count: 'exact', head: true }).eq('user_id', session.user.id)
