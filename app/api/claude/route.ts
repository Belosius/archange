...
async function callWithRetry(params: any, retries = 2): Promise<string> {
  ...
  throw new Error('Serveurs surcharges, reessayez dans quelques instants.')
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (action === 'raw') {
      ...
      const response = await callWithRetry({ max_tokens: 1024, system: system || 'Tu es un assistant utile.', messages: [{ role: 'user', content }] })
